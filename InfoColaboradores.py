...
class colabinfo:
    ...
    def __init__(self,pw):
        ...
        self.lab('Informacion Colaboradores', self.fuenteG, self.bglabel, self.fglabel, 490, 10)
    ...
    def mostrarDatos(self,where=""):
        ...
        if len(where)>0:
            cur=self.consultaBD("SELECT colaborador.id,colaborador.nombre, colaborador.genero, colaborador.fechanacimiento, infoemergencia.tiposangre, infoemergencia.encargado FROM iglesia.colaborador JOIN iglesia.infoemergencia ON colaborador.id = infoemergencia.colaborador_id " + where)
        else:
            cur=self.consultaBD("SELECT colaborador.id,colaborador.nombre, colaborador.genero, colaborador.fechanacimiento, infoemergencia.tiposangre, infoemergencia.encargado FROM iglesia.colaborador JOIN iglesia.infoemergencia ON colaborador.id = infoemergencia.colaborador_id ")
    ...
    def doubleClickTabla(self,event):
        ...
        cur=self.consultaBD("SELECT Colaborador.nombre, Colaborador.genero, Colaborador.fechanacimiento, Colaborador.tiposangre, Colaborador.encargado FROM iglesia.Colaborador JOIN iglesia.infoemergencia ON Colaborador.id = infoemergencia.colaborador_id WHERE Colaborador.id = '" + self.idViejo + "';")
        ...
        cur=self.consultaBD("SELECT telefono.id, telefono.telefono FROM iglesia.telefono JOIN iglesia.infoemergencia ON telefono.infoemergencia_id = infoemergencia.id JOIN iglesia.colaborador ON infoemergencia.colaborador_id = colaborador.id WHERE colaborador.id = '" + self.idViejo + "';")
        for (id, telefono) in cur:
            self.tablatel.insert('',0,text=id,values=telefono)

        cur=self.consultaBD("SELECT alergia.id, alergia.detalle FROM iglesia.alergia JOIN iglesia.infoemergencia ON alergia.infoemergencia_id = infoemergencia.id JOIN iglesia.colaborador ON infoemergencia.colaborador_id = colaborador.id WHERE colaborador.id = '" + self.idViejo + "';")
    ...
    def NomSearch(self):
        ...
        if len(self.buscar.get())>0:
            where = where + "and colaborador.nombre='" + self.buscar.get() + "' "
    ...
    def GenSearch(self):
        ...
        if len(self.comboS.get())>0:
            where = where + "and colaborador.genero='" + self.comboS.get() + "' "
        self.mostrarDatos(where)
    ...
    def FechaSearch(self):
        where=" where 1=1 "
        if len(self.buscar.get())>0:
            where = where + "and (YEAR(NOW()) - YEAR(colaborador.fechanacimiento)) = '" + self.buscar.get() + "' "
        self.mostrarDatos(where)
