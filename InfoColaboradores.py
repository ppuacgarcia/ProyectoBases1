...
class colabinfo:
    ...
    def mostrarDatos(self,where=""):
        ...
        if len(where)>0:
            cur=self.conn.consultaBD("SELECT colaborador.id,colaborador.nombre, colaborador.genero, colaborador.fechanacimiento, infoemergencia.tiposangre, infoemergencia.encargado FROM iglesia.colaborador JOIN iglesia.infoemergencia ON colaborador.id = infoemergencia.colaborador_id " + where)
        else:
            cur=self.conn.consultaBD("SELECT colaborador.id,colaborador.nombre, colaborador.genero, colaborador.fechanacimiento, infoemergencia.tiposangre, infoemergencia.encargado FROM iglesia.colaborador JOIN iglesia.infoemergencia ON colaborador.id = infoemergencia.colaborador_id ")
    ...
    def doubleClickTabla(self,event):
        ...
        cur=self.conn.consultaBD("SELECT telefono.id, telefono.telefono FROM iglesia.telefono JOIN iglesia.infoemergencia ON telefono.infoemergencia_id = infoemergencia.id JOIN iglesia.colaborador ON infoemergencia.colaborador_id = colaborador.id WHERE colaborador.id = '" + self.idViejo + "';")
        for (id, telefono) in cur:
            self.tablatel.insert('',0,text=id,values=telefono)

        cur=self.conn.consultaBD("SELECT alergia.id, alergia.detalle FROM iglesia.alergia JOIN iglesia.infoemergencia ON alergia.infoemergencia_id = infoemergencia.id JOIN iglesia.colaborador ON infoemergencia.colaborador_id = colaborador.id WHERE colaborador.id = '" + self.idViejo + "';")
